...
(
    ...
    private IEnumerable<(string ColumnName, string SqlType)> GetColumnNamesAndTypes(ICollection<ColumnPropertyInfo> columnProperties)
    {
        ...
        {
            if (columnProperty.SqlType != null)
            {
                yield return (columnProperty.ColumnName, columnProperty.SqlType);
                continue;
            }
            ...
            targetProps = props
                ...
                (
                    p =>
                    {
                        ...
                        var sqlType = targetAttr.NamedArguments
                            .FirstOrDefault(x => x.Key == nameof(ColumnInfoAttribute.SqlType))
                            .Value.Value as string;

                        propertyIndex++;

                        return new ColumnPropertyInfo(ordinal, columnName, p.Name, p.Type, numericPrecision, numericScale, length, sqlType);
                    }
                )
                .OrderBy(p => p.Ordinal);
        }
